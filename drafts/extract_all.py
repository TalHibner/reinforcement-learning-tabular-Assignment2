...
# Extract all 5 images first to specific names
img_idx = 0
image_names = [
    "img_empty_initial.png",
    "img_keydoor_initial.png",
    "img_empty_train.png",
    "img_keydoor_train.png",
    "img_optuna.png"
]
    ...
    if cell['cell_type'] == 'code' and 'outputs' in cell:
        for out in cell['outputs']:
            if 'data' in out and 'image/png' in out['data']:
                ...
                # Skip tiny video frames if any, but keep the 5 main plots
                if len(img_data) > 5000:
                    if img_idx < 5:
                        with open(image_names[img_idx], 'wb') as img_f:
...
md_lines = [
    ...
    "\n**EmptyEnv Initial State**",
    "![EmptyEnv Initial State](img_empty_initial.png)",
    "\n**EmptyEnv Training Progress**",
    "![EmptyEnv Plot](img_empty_train.png)",
    "\n# Environment 2: KeyDoorLavaEnv",
    "\n**KeyDoorLavaEnv Initial State**",
    "![KeyDoorLava Initial State](img_keydoor_initial.png)",
    "\n**KeyDoorLavaEnv Training Progress**",
    "![KeyDoorLavaEnv Plot](img_keydoor_train.png)",
    "\n```text",
    "============================================================",
...
for cell in nb['cells']:
    if cell['cell_type'] == 'markdown':
        ...
        if "8. Discussion" in src or "8.1" in src or "8.2" in src or "8.3" in src or "8.4" in src or "8.5" in src or "9. Best Settings" in src:
            ...
            # Inject Optuna Graph into Section 8.5
            target_optuna = "worst-case total penalty over 512 steps is only 0.256 — well below the 3.0 key bonus, so the signal is never overwhelmed."
            if target_optuna in src:
                src = src.replace(target_optuna, target_optuna + "\n\n![Optuna Hyperparameter Optimization](img_optuna.png)\n")
...
print("Markdown rebuilt with all 5 figures.")
